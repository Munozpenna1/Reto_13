...
```
def imprimir_valores_de_forma_ascendente(dNumeros):  
    valores = sorted(Numeros.values()) 
...
imprimir_valores_de_forma_ascendente(Numeros)
```
...
```
def mezclar_diccionarios(a, b): #Se define la funcion
    Mix = dict(b)  # Crear una copia del segundo diccionario

    for clave, valor in a.items(): #iteramos sobre las claves y valores del primer diccionario
        if clave not in Mix: 
            Mix[clave] = valor
...
diccionario_mejclado = mezclar_diccionarios(diccionario1, diccionario2)
print(diccionario_mejclado)
```
...
campos_alerta = ['alertAlertas', 'alertPrecip', 'alertTmpMax', 'alertTmpMin', 'alertVelViento']
...
for i in range(len(data['dt'])):
    for campo in campos_alerta:
        if data[campo][str(i)] == 'X':
            fecha = datetime.utcfromtimestamp(data['dt'][str(i)]).strftime('%Y-%m-%d')
            fechas_alerta.append(fecha)
            tipos_alerta.append(campo[5:])  # Eliminar "alert" del nombre del campo
            parametros_alerta.append(data[campo[5:]][str(i)])

for fecha, tipo, parametro in zip(fechas_alerta, tipos_alerta, parametros_alerta):
    print("Fecha de alerta:", fecha)
    print("Tipo de alerta:", tipo)
...
obtener_datos("https://api.genshin.dev/")

print("Dolar")
obtener_datos("https://api.exchangerate-api.com/v4/latest/USD")

print("Tabla periodica")
obtener_datos("https://neelpatel05.pythonanywhere.com/")
```
